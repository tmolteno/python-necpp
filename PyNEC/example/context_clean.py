...
class context_clean(object):
    ...
    # TODO: multiplicative
    def set_frequencies_linear(
        self, start_frequency, stop_frequency, count=None, step_size=None
    ):
    ...
    def set_perfect_ground(self):
        ...
        self.context.gn_card(
            gn_perfectly_conducting, no_ground_screen, 0, 0, 0, 0, 0, 0
        )
    ...
    def radiation_pattern(
        ...
        thetas,
        phis,
        output_mode=output_vertical_horizontal,
        normalization=normalization_none,
        gain=power_gain,
        average=average_todo,
    ):
    ...
    def transmission_line(
        ...
        crossed_line=False,
        length=None,
        shunt_admittance_src=0,
        shunt_admittance_dst=0,
    ):
        ...
        debug(
            ...
            impedance,
            length,
            shunt_admittance_src.real,
            shunt_admittance_src.imag,
            shunt_admittance_dst.real,
            shunt_admittance_dst.imag,
        )
        self.context.tl_card(
            src[0],
            src[1],
            dst[0],
            dst[1],
...
class geometry_clean(object):
    ...
    def wire(
        self, tag_id, nr_segments, src, dst, radius, length_ratio=1.0, radius_ratio=1.0
    ):
        ...
        if nr_segments < 2:
            raise ValueError(
                f"segment_count must be >= 2 (got {nr_segments}). NEC2 requires at least 2 segments per wire."
            )
        # Cast to native float in case numpy scalars are passed (SWIG may not convert them)
        xw1, yw1, zw1 = float(src[0]), float(src[1]), float(src[2])
        xw2, yw2, zw2 = float(dst[0]), float(dst[1]), float(dst[2])
        rad = float(radius)
        debug("GW", tag_id, nr_segments, xw1, yw1, zw1, xw2, yw2, zw2, rad)  # TODO
